...
function htmlTemplate({
  ...
  // Questions are already sorted by the Lexia test interface order
  // No need to sort again - they come in the correct sequence
  ...
  // Display questions in pure global order - just like the test interface
  ...
  const questionBlocks = sortedQuestions
    .map((q) => {
      ...
      // Show passage only if this question has a different passage than the previous one
...
export async function GET(request, { params }) {
  try {
    ...
    // Fetch Lexia test (user-generated) by id
    const lexia = await prisma.lexiaTest.findUnique({
      where: { id },
    })
    ...
    const isOwner = session.user?.id === lexia.userId
    const isAdmin = session.user?.role === 'ADMIN'
    if (!isOwner && !isAdmin) {
      return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
    }

    // Load passages and questions referenced by the Lexia test
    const [passages, questions] = await Promise.all([
      prisma.passage.findMany({
        where: { id: { in: lexia.passageIds || [] } },
      }),
      prisma.question.findMany({
        where: { id: { in: lexia.questionIds || [] } },
      }),
    ])

    // Build a test-like object for template compatibility
    const test = {
      id: lexia.id,
      title: lexia.title,
      type: lexia.type, // MOCK/SECTIONAL, shown in badge
      durationInMinutes: lexia.durationInMinutes,
    }

    // Sort questions based on the order in lexiaTest.questionIds and assign sequential numbers
    // This matches exactly how the Lexia test interface displays questions
    const sortedQuestions = lexia.questionIds
      .map((id, index) => {
    ...
    // Build order maps from LexiaTest arrays to preserve exact sequencing
    const passageOrder = new Map()
    ;(lexia.passageIds || []).forEach((pid, idx) => passageOrder.set(pid, idx))
    const questionOrder = new Map()
    ;(lexia.questionIds || []).forEach((qid, idx) =>
      questionOrder.set(qid, idx)
    )

    const html = htmlTemplate({
      test,
      passages: passages || [],
      questions: sortedQuestions, // Use the correctly ordered questions
      passageOrder,
      questionOrder,
    })
