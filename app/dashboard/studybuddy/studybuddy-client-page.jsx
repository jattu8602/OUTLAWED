...
export default function StudyBuddyClientPage() {
  ...
  useEffect(() => {
    const loadChatHistory = async () => {
      try {
        const response = await fetch('/api/studybuddy/history')
        if (response.ok) {
          const data = await response.json()
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await fetch('/api/studybuddy/chat', {
        method: 'POST',
        headers: {
          ...
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center px-4">
              ...
              <p className="text-slate-600 dark:text-slate-400 mb-6 max-w-md leading-relaxed">
                I'm here to help you with concepts, test preparation, and
                study guidance. Ask me anything!
              </p>
              ...
              <Input
                ...
                placeholder={
                  selectedTest
                    ...
                    : 'Ask me anything... (Type @ to reference a test)'
                }
                disabled={isLoading}
